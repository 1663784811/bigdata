...
public class WxPortalController {
    ...
    public String authGet(
        ...
        if (StringUtils.isAnyBlank(signature, timestamp, nonce, echostr)) {
            WebException.fail("请求参数非法，请核实!");
        } else if (!this.wxService.switchover(appid)) {
            WebException.fail(String.format("未找到对应appid=[%s]的配置，请核实！", appid));
    ...
    public String post(
        ...
        if (!this.wxService.switchover(appid)) {
            ...
            WebException.fail("非法请求，可能属于伪造的请求！");
        }
        String out = null;
        if (encType == null) {
            // 明文传输的消息
