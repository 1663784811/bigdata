 ...
 * <p>
 * 在曲面上显示视频流。
 * renderFrame（）是异步的，以避免阻塞调用线程。
 * 此类是线程安全的，可处理可能来自三个不同线程的访问：
 * init、release和setMirror中主应用程序的交互。在曲面上显示视频流。
 * renderFrame（）是异步的，以避免阻塞调用线程。
 * 此类是线程安全的，可处理可能来自三个不同线程的访问：
 * init、release和setMirror中主应用程序的交互。
 * renderFrame中来自C++rtc:：VideoSinkInterface的交互。
 * 在surfaceCreated、surfaceChanged和surfaceDestroyed中，来自SurfaceHolder生命周期的交互。
 * renderFrame中来自C++rtc:：VideoSinkInterface的交互。
 * 在surfaceCreated、surfaceChanged和surfaceDestroyed中，来自SurfaceHolder生命周期的交互。
 */
