...
class VideoDownloader:
    ...
    def download_single_video(self, link):
        if len(link.items()) > 3:
            ...
            pbar = tqdm(total=len(download_links), desc=f'正在下载[{mp4_name}]')
            temp_folder_path = self.downloader(download_links=download_links, path_names=path_names, pbar=pbar)
    ...
    def downloader(self, download_links, path_names, pbar):
        temp_folder_path = os.path.join(self.absolute_path, 'temp')
        os.makedirs(temp_folder_path, exist_ok=True)

        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_thread_number) as executor:
            # as_completed
            futures = [executor.submit(self.download_ts, link, name, temp_folder_path, pbar) for link, name in
    ...
    def download_ts(self, link, name, temp_folder_path, pbar):
        ...
        if self.proxy_code == 1:
            try:
                ...
                pbar.update(1)
                pbar.set_postfix_str(f'正在下载视频片段{name}')
                # print(f'来自 {link} 的文件 {name} 下载完成!')
            except requests.exceptions.RequestException as e:
                print(f"Error in downloading ts file {name}: {e}")
        else:
            try:
                response = self.session.get(link, headers=self.headers, proxies=self.proxies)
                response.raise_for_status()
                with open(ts_path, 'wb') as file:
                    file.write(response.content)
                pbar.update(1)
...
if __name__ == '__main__':
    target = 'https://www.yhdmdm.com/tv/dm16075.html'
